...
class Buoy {
	...
	public function isValid() {
		// last 15 days of data from buoy
		$this->data = file('http://www.ndbc.noaa.gov/data/realtime2/' . $this->buoyId . '.txt');
		if ($this->data)
			return TRUE;
		else 
			return FALSE;	
	...
	public function hasAccurateData($obsdate) {
		...
		foreach ($this->data as $key => $line) { 
			...
			if ($key > 1) {
				...
				if (abs($obsdate - $buoyDate) < 60*60*4) {
					$proximity[$key] = abs($obsdate - $buoyDate);
